...
class GenericEntryListWidget(TemplateTag):
    ...
    options = Options(
        ...
        'with_manager', Argument('model_manager', required=False, resolve=False),
        'filter_by', Argument('filter_by', required=False, resolve=False),
        'limit', Argument('limit', required=False, resolve=False),
        'order_by', Argument('order_by', required=False, resolve=False),
        'using', Argument('template', required=False, resolve=False),
        'as', Argument('varname', required=False, resolve=False),
    )

    def render_tag(self, context, model, model_manager, filter_by, limit, template, order_by, varname):  # noqa
        try:
            ...
            logger.exception("Model '{0}' not found with using template tag show_elements_list".format(model))
        if not model_manager:
            model_manager = 'objects'
        filter_kwargs = dict(([x.__str__() for x in filter_by.split('=')],)) if filter_by else {}
        ...
        if varname:
            ...
            tag_context = Context({'object_list': entry_qs,
...
class GetListWidget(TemplateTag):
    ...
    options = Options(
        ...
        'as', Argument('varname', required=True, resolve=False),
    )
